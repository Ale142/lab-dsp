...
const TaskRowData = ({ task, onCheck, filter, owner }) => {
  const labelClasses = [
      ...
      <ListGroup>
        {tasks.map((task) => (
          ...
          >
            <TaskRowData
              task={task}
              onCheck={(flag) => { onCheck(task, flag); console.log("Selected task:" + task) }}
              filter={filter}
              owner={filter === 'assigned' ? assignedTaskList.filter(t => String(t.taskId) === String(task.id))[0]?.userName : ""}
            />
            <TaskRowControl
              task={task}
