...
async function getAssignmentsStatus() {
  return new Promise((resolve, reject) => {
    fetch(BASEURL + "/tasks/assignments/status")
      .then(response => {
        if (response.ok) {
          resolve(null);
        }
...
const API = { addTask, getTasks, getPublicTasks, getAllOwnedTasks, updateTask, deleteTask, selectTask, logIn, logOut, getUserInfo, getUsers, assignTask, removeAssignTask, completeTask, getAssignmentsStatus }
export default API;
