...
module.exports.getAssignmentsStatus = function getAssignmentsStatus(req, res, next) {
    Tasks.getAssignmentsStatus()
        .then(response => {
            utils.writeJson(res, response, 204);
        })
        .catch(function (response) {
            console.log("getAssignmentsStatus (Tasks.js):", response);
            utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': response }] }, 500)
        })
}